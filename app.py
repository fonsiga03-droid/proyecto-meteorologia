...
COLORS = {
    "Sevilla": "#e63946", "Madrid": "#457b9d", "Barcelona": "#2a9d8f",
    "Valencia": "#f4a261", "Bilbao": "#264653", "Almeria": "#e9c46a",
    "Zaragoza": "#a8dadc", "Malaga": "#e76f51",
}
...
st.markdown("**Proyecto Final Big Data** · Línea 8 · NASA POWER 2022–2023 · 8 ciudades · 140,160 registros")
...
resumen = df.groupby("location").agg(
    ...
    lat=("lat", "first"),
    lon=("lon", "first"),
).reset_index().sort_values("P_anual", ascending=False)

col1, col2, col3, col4 = st.columns(4)
col1.metric("🏆 Ciudad más solar", resumen.iloc[0]["location"],
            f"{resumen.iloc[0]['GHI_mean']:.0f} W/m² media")
col2.metric("⚡ Producción máxima", f"{resumen.iloc[0]['P_anual']:.0f} kWh/año",
            "por panel de 340W")
col3.metric("📉 Ciudad menos solar", resumen.iloc[-1]["location"],
            f"{resumen.iloc[-1]['GHI_mean']:.0f} W/m² media")
col4.metric("📊 Diferencia Norte-Sur",
...
with tab1:
    ...
    fig.update_geos(
        lataxis_range=[35, 45], lonaxis_range=[-10, 5],
...
with tab2:
    ...
    fig.update_layout(
        xaxis_title="Mes", yaxis_title="GHI (W/m²)",
        height=420, hovermode="x unified",
        legend=dict(orientation="h", yanchor="bottom", y=1.02),
    )
    st.plotly_chart(fig, use_container_width=True)
    st.caption("Fuente: NASA POWER · Datos horarios 2022–2023 · Resolución espacial 0.5°")
...
with tab3:
    ...
    variable = col_b.selectbox("Variable", {
    ...
    with subtab1:
        ...
        fig = px.bar(monthly_city, x="mes", y=variable,
                     color_discrete_sequence=[COLORS[ciudad]])
        fig.update_layout(height=350, xaxis_title="Mes",
                          yaxis_title=variable, showlegend=False)
        st.plotly_chart(fig, use_container_width=True)

    with subtab2:
        hourly_city = ciudad_df.groupby(["hour", "month"])[variable].mean().reset_index()
        ...
        for mes in ["Ene", "Mar", "Jun", "Sep", "Dic"]:
            if mes in hourly_pivot.columns:
                fig.add_trace(go.Scatter(
                    x=hourly_pivot.index, y=hourly_pivot[mes],
                    mode="lines", name=mes, fill="tozeroy", opacity=0.6,
                ))
        fig.update_layout(
            height=350, xaxis_title="Hora UTC",
            yaxis_title=variable, hovermode="x unified",
        )
        st.plotly_chart(fig, use_container_width=True)
...
with tab4:
    ...
    for col, (_, row) in zip(cols, prod_anual.sort_values("Ahorro_anual_€", ascending=False).iterrows()):
        col.metric(row["location"], f"{row['Ahorro_anual_€']:.0f} €/año",
                   f"{row['P_total_kwh']:.0f} kWh")

st.divider()
st.caption("Datos: NASA POWER API · Modelo: POA isotrópico, NOCT térmico · Panel: 340W, η=20%, 1.7m² · Proyecto Final Big Data 2025–2026")
